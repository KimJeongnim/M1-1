...
2026년 데이터는 2026년 1월부터 7월까지 별도로 수집한 17개 지역의 데이터를 통합하여 총 119개 관측값으로 구성하였다.
...
## 5.5 2023~2025년 월별 평균 관광객 수 증감률

![월별 평균 관광객 증감률](images/05_monthly_change_rate.png)
...
## 인사이트 5. 월별 방문자 증가율은 일정하지 않고 변동성이 나타났다.
...
증가폭이 상대적으로 큰 지역은 세종, 제주, 충남 등이었으며, 울산과 경남 등은 감소하였다.
...
## 8.1 분석 목적
2023~2025년의 장기적인 관광 방문 추세를 학습한 시계열 예측 모델을 생성하고, 2026년 1~7월의 실제 데이터와 비교하여 예측 모델의 성능과 최근 관광 수요의 흐름을 평가하였다.

---

## 8.2 예측 모델 개요
* **사용 모델**: Holt-Winters Exponential Smoothing (계절성 시계열 예측)
...
## 8.3 성능 평가 결과
예측 모델과 2026년 실제 데이터 간의 오차를 측정한 결과는 다음과 같다.
...
지역별로는 17개 지역 중 15개 지역의 2023년 대비 2025년 평균 방문자 수가 증가했지만, 증가율에는 지역별 차이가 있었다. 특히 대전, 세종, 대구, 부산 등의 증가율이 높았으며 강원과 제주는 감소하였다.
...
1. 본 데이터는 관광 방문자 수의 추정값을 기반으로 하므로 절대적인 실제 관광객 수로 해석하기보다는 관광 흐름과 추세를 파악하는 지표로 활용할 필요가 있다.
...
데이터 해석 시 관광 데이터랩에서 제공하는 데이터의 집계 및 추정 특성을 고려하였다.
...
AI가 제시한 분석 결과를 그대로 사용하지 않고 Python을 이용하여 데이터와 계산 결과를 확인한 후 최종 인사이트를 작성하였다.
